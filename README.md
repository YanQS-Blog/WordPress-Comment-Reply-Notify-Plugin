...
- 每个模板现在支持多套风格（Modern / Plain / Compact），可在后台「邮件模板」页为 `reply`、`new_comment`、`pending` 单独选择风格。
- 选择风格后可使用「载入所选风格」将该风格的模板载入编辑器，便于快速套用样式；也可直接编辑自定义模板并保存。
- 在编辑器中点击「预览」可打开邮件预览弹窗，使用示例数据渲染模板以便查看最终效果（出于安全考虑，预览会移除并忽略模板内的 PHP 代码以避免在后台执行）。
- 风格模板文件位于 `includes/templates/styles/{modern,plain,compact}/`，保存后的自定义模板会尝试写入 `includes/templates/`，写入失败时回退至数据库选项 `pcn_templates`。

注意：模板文件在真实发送邮件时会被 `includes/class-pcn-mailer.php` 中的渲染逻辑包含并执行（如果模板文件包含 PHP 则会运行），因此请仅在可信任的环境中使用 PHP 代码。
